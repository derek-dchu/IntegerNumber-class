...
int main(int argc, char * argv[]) {
	...
	IntegerNumber C = "5678954688709764347890"; // IntegerNumber object C is created and C contains the positive number shown within the quotes " "
	IntegerNumber D(B); // IntegerNumber object D is created and D contains the number that B contains
	IntegerNumber E(B); // IntegerNumber object E is created and E contains the number that B contains
	A = B; // assigns the value of A to that of B
	cout << A << " " << B << endl; // output to screen the integers in A and B
	C = B + C;
	cout << C << endl; // output to screen the sum of the numbers B and C
	D = B - C;
